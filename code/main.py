...
def main():
    ...
    parser.add_argument('--arch', default='VGG16', help='Architecture to use') 
...
class Solver(object):
    ...
    def load_model(self):
        ...
        self.model = args.arch.to(self.device)
    ...
    def train(self):
        ...
        for batch_num, (data, target) in enumerate(self.train_loader):
            ...
            train_correct += np.sum(prediction[1].cpu().numpy() == target.cpu().numpy())
                ...
                #print('Loss: %.4f | Acc: %.3f%% ' % (train_loss / (500), 100. * train_correct / 500))
            progress_bar(batch_num, len(self.train_loader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'% (train_loss / (batch_num + 1), 100. * train_correct / total, train_correct, total))
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            progress_bar(batch_num, len(self.test_loader), 'Loss: %.4f | Acc: %.3f%% (%d/%d)'% (test_loss / (batch_num + 1), 100. * test_correct / total, test_correct, total))
            #if (batch_num+1)%500 == 499:
                #print('Loss: %.4f | Acc: %.3f%% ' % (test_loss / (500), 100. * test_correct / 500))  
