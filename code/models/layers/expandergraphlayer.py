...
class expanderLinear(Function):
    ...
    def forward(self, input, weight):
        ...
        extendWeights = weight.clone().cuda()
...
class ExpanderLinear(torch.nn.Module):
    def __init__(self, input_features, output_features, expandSize, mode='random'):
        ...
        self.weight = torch.Tensor(output_features, input_features)
        ...
        self.mask =  self.mask.cuda().float()
...
class ExpanderConv2d(torch.nn.Module):
    def __init__(self, inWCin, inWCout, kernel_size, expandSize,stride=1, padding=0, inDil=1, groups=1, mode='random'):
        ...
        self.fpWeight = torch.nn.Parameter(data=torch.Tensor(inWCout, inWCin, kernel_size, kernel_size), requires_grad=True).float()
